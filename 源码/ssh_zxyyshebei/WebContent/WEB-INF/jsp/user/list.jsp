            ...
            <form method="post" action="user_list.do">
                <div class="search_style">
                    <ul class="search_content clearfix">
                        <li><input name="name" type="text" value="${bean.name}"
                                                                   class="text_add" placeholder="登录账号" style=" width:150px"/></li>
                        <li><input name="number" type="text" value="${bean.number}"
                                                                 class="text_add" style=" width:150px" placeholder="学号/教师号"/></li>
                        <li><input name="phone" type="text" value="${bean.phone}"
                                                                    class="text_add" style=" width:150px" placeholder="手机号码"/></li>
                        <li><input name="bj" type="text" value="${bean.bj}"
                                                                    class="text_add" style=" width:150px" placeholder="班级"/></li>
                        <li><input name="realName" type="text" value="${bean.realName}"
                                                                    class="text_add" style=" width:150px" placeholder="真实姓名"/></li>
                        <li>
...
<c:if test="${role.enName == 'admin'}">
       ...
       <span class="l_f">
        <button type="submit" id="member_add" class="btn btn-warning" onclick="add()"><i class="icon-plus"></i>添加用户</button>
       </span>
        </div>
    </c:if>
    <div class="margin clearfix">
        ...
        <div class="pagelist">
            <pg:pager url="${ctx}/user_list.do" maxIndexPages="5" items="${pagers.total}" maxPageItems="15"
                ...
                <pg:param name="number" value="${bean.number}"/>
                <pg:param name="phone" value="${bean.phone}"/>
                <pg:param name="bj" value="${bean.bj}"/>
...
<script type="text/javascript">
    function edit(id) {
        layer.open({
            type: 2,
            area: ['910px', '500px'],
            content: ['user_edit.do?userId=' + id, 'no']  //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
